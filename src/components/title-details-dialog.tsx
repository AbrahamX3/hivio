...
function TrailerSection({
	...
	videos: Array<{ key: string; name: string }>;
	...
	return (
		<div className="space-y-6">
			...
			<div className="group border-primary/10 hover:border-primary/30 relative aspect-video w-full overflow-hidden rounded-3xl border bg-gray-950 shadow-2xl transition-all">
				<LiteYouTubeEmbed
					id={videos[0].key}
					title={videos[0].name}
			...
			{isLoadingDetails ? (
				...
				<div className="grid gap-4">
					{details.seasons.map((season) => {
						...
						const isSeasonExpanded = expandedSeasons.has(season.seasonNumber);
						const isLoadingSeason = loadingSeasons.has(season.seasonNumber);
						...
						return (
							<Collapsible
								...
								onOpenChange={(open) =>
									onSeasonToggle(season.seasonNumber, open)
								}
...
function AddToWatchlistBar({
	...
	if (!showAddToWatchlist || (!onAddToWatchlist && !onOpenAddTitleDialog)) {
		return null;
	}
...
export function TitleDetailsDialog({
	...
	const handleSeasonToggle = async (
		...
		try {
			const episodes = await client.tmdb.getSeasonEpisodes({
				tmdbId: title.tmdbId,
				seasonNumber,
			});
			...
			<DialogContent className="flex h-[95vh] flex-col overflow-hidden p-0 sm:max-w-5xl">
				...
				<ScrollArea className="flex-1">
					<div className="relative pb-20">
						{title.backdropUrl && (
							<div className="absolute inset-0 -z-10 h-125 overflow-hidden">
								<Image
									loader={tmdbImageLoader}
									src={title.backdropUrl}
									alt=""
									fill
									sizes="(max-width: 639px) 100vw, min(1024px, 100vw)"
						...
						<div className="p-6 md:p-10">
							...
							<div className="grid grid-cols-1 gap-12 lg:grid-cols-[1fr_320px]">
								...
								<SidebarInfo
									...
									displayDirectors={displayDirectors}
								/>
							</div>
						</div>
					</div>
				...
				<AddToWatchlistBar
					...
					onOpenAddTitleDialog={onOpenAddTitleDialog}
					title={title}
					setOpen={setOpen}
				/>
			</DialogContent>
