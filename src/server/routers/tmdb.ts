...
export const tmdbRouter = {
	...
	getNextEpisodeInfo: publicProcedure
		.input(
			z.object({
				tmdbId: z.number(),
		...
		.handler(async ({ input }) => {
			const cacheKey = `tmdb:nextEpisode:${input.tmdbId}:${input.currentSeason}:${input.currentEpisode}`;
			const cached = await redis.get<{
				nextEpisode: {
			...
			try {
				...
				const nextEpisode = season.episodes.find(
					(ep) => ep.episode_number === input.currentEpisode + 1,
				);
