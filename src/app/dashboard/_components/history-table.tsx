...
function historyColumnFiltersToApi(
	...
	return columnFilters
		.map((cf) => {
			...
			const variant = col?.meta?.variant ?? "text";
			if (variant === "multiSelect") {
				const value = cf.value as string[];
...
export function useHistoryTable({ onEdit, onDelete }: HistoryTableStateProps) {
	...
	const result = useQuery({
		queryKey: ["history", "getAll", queryArgs],
		queryFn: () =>
			client.history.getAll({
				filters: queryArgs.filters as Parameters<
					typeof client.history.getAll
				>[0]["filters"],
				sort: queryArgs.sort,
				page: queryArgs.page,
